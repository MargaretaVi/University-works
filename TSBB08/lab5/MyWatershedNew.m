...
figure(9), imshow(D2,[],'InitialMagnification','fit');
title('Distance transform of bw');
colormap(jet), colorbar;
...
figure(10), imshow(change,[],'InitialMagnification','fit');
title('change');
colormap(jet), colorbar;
change(1,1) = 1; 
...
D2u = uint8(~D2);
added =  D2u + change;
figure(11), imshow(added,[],'InitialMagnification','fit');
title('Figure 2 and figure 9 ');
colormap(jet), colorbar;
%%

% Perform labeling
%-----------------
labelstructchange = bwconncomp(added,8); 

% Make a labelimage to look at
%-----------------------------
NumObj = labelstructchange.NumObjects;
labelim = zeros(labelstructchange.ImageSize);
for no = 1:NumObj
  labelimnew(labelstructchange.PixelIdxList{no}) = no;
end
%figure(12), imshow(labelimnew,[],'InitialMagnification','fit');
colormap(jet), colorbar;
title('labeling of regional min');

% Compute the watershed transform
%--------------------------------
W1new = watershed_meyer(added,8,labelstructchange);
figure(13), imshow(W1new,[],'InitialMagnification','fit');
colormap(jet), colorbar;
title('Watershed of Dinv');
