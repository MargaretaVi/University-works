%function [xhat, meas] = filterTemplate(calAcc, calGyr, calMag,gyro_mean,gyro_cov)
...
while server.status()  % Repeat while data is available
    ...
    if ~any(isnan(gyr))  % Gyro measurements are available.
        % Do something
        [x,P] = tw_qw(x, P,  meas.t(end)-meas.t(end-1), 1000*gyro_cov);
        kgyr = 1;
        meas.t(end)-meas.t(end-1)
    else
        kgyr = kgyr+1;
    end
    ...
    if ~any(isnan(acc))  % Acc measurements are available.
        if (norm(acc,2) < 11 && norm(acc,2) > 8)
            [x,P] = mu_g(x, P, acc, 0.1*acc_cov, [0; 0; 9.82]);
    ...
    if ~any(isnan(mag))  % Mag measurements are available.
        if (abs(norm(mag,2) - norm(mag_mean,2)) < 20)
            [x,P] = mu_m(x, P, mag, 0.1*mag_cov, [0 sqrt(mag_mean(1)^2 +mag_mean(2)^2) , mag_mean(3)]');
